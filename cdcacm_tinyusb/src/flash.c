...
FlashStatus Flash_ErasePage(uint32_t current_page) {
...
FlashStatus Flash_ProgramDword(uint32_t dst_address, void *src_buffer) {
    // Check alignment
...
FlashStatus Flash_ProgramRow(uint32_t dst_address, void *src_buffer) {
    //Check alignment
    if ((uint32_t)src_buffer % 8 != 0) {
        return FLASH_ERR_ALIGNMENT_SRC;  // Must be 64-bit aligned
    }
    ...
    // 1. page erase (outside of this function)
    
    // 2. Wait if busy
    while (FLASH->SR & FLASH_SR_BSY) {};

    // 3. clear all previous programming error flags
    FLASH->SR = (FLASH_SR_OPTVERR | 
                 FLASH_SR_RDERR | 
                 FLASH_SR_FASTERR | 
                 FLASH_SR_MISERR | 
    ...
    // 4. check that CFGBSY bit of FLASH_SR is cleared
    while (FLASH->SR & FLASH_SR_CFGBSY) {};
    
    // 5. set the PG bit of the FLASH_SR
    FLASH->CR |= FLASH_CR_FSTPG;
    
    // 6. perform data write
    // This is the **key step**: write 256 bytes at once
    memcpy((void *)dst_address, src_buffer, FLASH_ROW_SIZE);
    //*(uint64_t*)dst_address = *(uint64_t*)src_buffer;
    
    // 7. Wait until CFGSY bit FLASH_SR is cleared
    while (FLASH->SR & FLASH_SR_CFGBSY) {};

    // 8. Check that EOP flag of FLASH_SR is set (=programming success)
    // and clear it by software (only when EOPIE is enabled)
    // if (FLASH->SR & FLASH_SR_EOP){
        // FLASH->SR |= FLASH_SR_EOP;
    // }
    ...
    // 9. Clear the PG bit of FLASH_CR if no more programming is needed
    FLASH->CR &= ~FLASH_CR_FSTPG;
...
FlashStatus Flash_ProgramFullPage(uint32_t current_page, void *source_data) {
    ...
    for (int i = 0; i < FLASH_PAGE_SIZE - FLASH_DWORD_SIZE; i += FLASH_DWORD_SIZE) {
